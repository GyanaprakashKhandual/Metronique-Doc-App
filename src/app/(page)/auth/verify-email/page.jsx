...
export default function Page() {
    ...
    const handleEmailVerification = async () => {
        try {
            ...
            if (!token || !email) {
                ...
                showAlert({ type: 'error', message: 'Invalid verification link' });
                setTimeout(() => router.push('/auth'), 3000);
                return;
            }

            const res = await fetch(`${API_BASE_URL}/auth/verify-email-address`, {
                method: 'POST',
            ...
            try {
                ...
                showAlert({ type: 'error', message: 'Invalid server response' });
                setTimeout(() => router.push('/auth'), 3000);
                return;
            }
            ...
            if (data.success) {
                ...
                showAlert({ type: 'success', message: 'Email verified successfully!' });
                setTimeout(() => {
                    router.push('/app');
                }, 2000);
            } else {
                ...
                showAlert({ type: 'error', message: data.message || 'Verification failed' });
                setTimeout(() => router.push('/auth'), 3000);
            }
        } catch (error) {
            setStatus('error');
            setMessage('An error occurred during verification');
            showAlert({ type: 'error', message: 'An error occurred during verification' });
            setTimeout(() => router.push('/auth'), 3000);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-linear-to-br from-white via-gray-50 to-white flex items-center justify-center p-4 relative overflow-hidden">
